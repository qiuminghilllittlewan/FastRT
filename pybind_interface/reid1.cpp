...
/* Ex1. sbs_R50-ibn */
static const std::string WEIGHTS_PATH = "../veri_wild_bot_R50_ibn.wts";
static const std::string ENGINE_PATH = "./veri_wild_bot_R50_ibn.engine";
...
ReID1::ReID1(int device): baseline(trt::ModelConfig {
        WEIGHTS_PATH,
        MAX_BATCH_SIZE,
        INPUT_H,
        INPUT_W,
...
int ReID1::build(const std::string &engine_file)
{
...
std::list<float> ReID1::infer(py::array_t<uint8_t>& img)
{
...
std::list<std::list<float>> ReID1::batch_infer(std::list<py::array_t<uint8_t>>& imgs)
{
    // auto t1 = Time::now();
    std::vector<cv::Mat> input;
    int count = 0;
...
PYBIND11_MODULE(ReID1, m) {
    m.doc() = R"pbdoc(
        Pybind11 example plugin
    )pbdoc";
    py::class_<ReID1>(m, "ReID1")
        .def(py::init<int>())
        .def("build", &ReID1::build)
        .def("infer", &ReID1::infer, py::return_value_policy::automatic)
        .def("batch_infer", &ReID1::batch_infer, py::return_value_policy::automatic)
        ;
